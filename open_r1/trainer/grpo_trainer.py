...
def simple_processor(tokenizer, texts, images, image_size=512, pad_image=True, padding_side="left",
    ...
    pixel_values = torch.stack([process_image(img) for img in images], dim=0)
...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                model_init_kwargs.pop("attn_implementation", None)
        ...
        if image_token not in processing_class.get_vocab():
            special_tokens_dict = {"additional_special_tokens": [image_token]}
            processing_class.add_special_tokens(special_tokens_dict)
            model.llm.resize_token_embeddings(len(processing_class))
            self.ref_model.llm.resize_token_embeddings(len(processing_class))

        model.IMAGE_TOKEN_INDEX = processing_class.convert_tokens_to_ids(image_token)
        self.ref_model.IMAGE_TOKEN_INDEX = processing_class.convert_tokens_to_ids(image_token)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        prompt_inputs = simple_processor(
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            output = unwrapped_model.generate(**prompt_inputs, generation_config=self.generation_config)
        ...
        pixel_values = pixel_values.unsqueeze(1).repeat(1, self.num_generations, 1, 1, 1)
